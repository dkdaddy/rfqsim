...
const express_1 = __importDefault(require("express")); // Added RequestHandler
...
const logger = {
    ...
    venue: (message, data) => console.log(`[VENUE_LOG] ${new Date().toISOString()} ${message}`, data || ''),
};
const wssConnections = new Map();
...
function startSimulator() {
    ...
    setInterval(() => {
        ...
        const currentInstrumentInstance = { ...instrumentTemplate };
        const newRfq = {
            id: `RFQ-${(0, uuid_1.v4)()}`,
        ...
        if (currentInstrumentInstance.instrumentType === 'Equity')
            newRfq.size = Math.floor(newRfq.size / 1000) * 100;
        rfqs.push(newRfq);
        ...
        if (instrumentIndex !== -1)
            instruments[instrumentIndex] = currentInstrumentInstance; // Update with potentially new DV01
...
app.get('/rfqs', (req, res) => {
    ...
    const responseData = filteredRfqs.map(getFullRfqDetails).filter(r => r !== null);
...
const respondToRfqHandler = (req, res, next) => {
    const rfq = rfqs.find(r => r.id === req.params.rfqId);
...
const passOnRfqHandler = (req, res, next) => {
    const rfq = rfqs.find(r => r.id === req.params.rfqId);
